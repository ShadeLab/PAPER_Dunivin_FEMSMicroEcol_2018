...
write.table(otu_table, paste(wd, "/output/otu_table_0.1.txt", sep = ""), sep = "\t", quote = FALSE, row.names = FALSE)
...
corr.genes <- corr.test(otu_table_norm.slim.t, method = "spearman", adjust = "fdr", alpha = 0.1)
...
blast.all.unique <- blast.all.unique %>%
  ...
  left_join(organisms.ncbi, by = "query") %>%
...
blast.final <- rbind(blast.rplB.tidy.unique, blast.all.unique)
...
#save taxize results to output so that ncbi does not need to be re-searched
write.table(blast.final, paste(wd, "/output/taxize_0.1_results_FINAL.txt", sep = ""), col.names = TRUE, row.names = FALSE)
...
#find correlations between contigs!
corr.genes.slim <- corr.test(otu_table_norm.slim.t.genes, method = "spearman", adjust = "fdr", alpha = 0.1)
